...
dirCheckpoints = dirRoot + '/code/DisVAE/checkpoints/disvae_rafd_svm'
...
load_encoderE = dirRoot + '/code/STVAE/checkpoints/pretrain_final/model/encoder_epoch_4.pth'
load_decoderE = dirRoot + '/code/STVAE/checkpoints/pretrain_final/model/decoder_epoch_4.pth'
...
batch_size = 64
parameters2 = list(encoderE.parameters()) + list(decoderE.parameters())
optimizer2 = optim.Adam(parameters2, lr=args.lr, betas=(args.beta1, 0.999))
alpha = 1.0
beta = 0.2
def train_facenet():
    iteration_count = 0
...
mix = mix.cuda()
parameters3 = list(encoderE.parameters()) + list(decoderE.parameters()) + list(mix.parameters())
...
def train():
    ...
    for epoch in range(args.nepoch):
        ...
        acc  = predict(train_subject, test_subject)
        print('acc is:', acc)
        if acc > bestacc:
            bestacc = acc
...
def predict(train_subject, test_subject):
    ...
    train_y = []
    for i, batch in enumerate(dataloader):
        img, label, _, path = batch[0].cuda(), batch[1].cuda(), batch[2], batch[3]
        _, mu, _ = encoderE(img)
    ...
    clf = svm.SVC(C=10, gamma=0.1, decision_function_shape='ovo')
