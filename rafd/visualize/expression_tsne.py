...
dirCheckpoints = dirRoot + '/code/DisVAE/checkpoints/disvae_rafd_mlp'
dirImageoutput = dirCheckpoints + '/train_oulu_img'
dirTestingoutput = dirCheckpoints + '/test_oulu_img'
dirModel = dirCheckpoints + '/model'
# ident 50, facenet 10
# load_encoderI = dirRoot + '/code/STVAE/checkpoints/pretrain_final/model/encoder_epoch_4.pth'
# load_decoderI = dirRoot + '/code/STVAE/checkpoints/pretrain_final/model/decoder_epoch_4.pth'
load_encoderI = dirRoot + '/code/DisVAE/checkpoints/disvae_rafd_mlp/model/encoderI_epoch_49.pth'
load_decoderI = dirRoot + '/code/DisVAE/checkpoints/disvae_rafd_mlp/model/decoderI_epoch_49.pth'
load_encoderE = dirRoot + '/code/DisVAE/checkpoints/disvae_rafd_mlp/model/encoderE_epoch_49.pth'
load_decoderE = dirRoot + '/code/DisVAE/checkpoints/disvae_rafd_mlp/model/decoderE_epoch_49.pth'
...
class rafdPairLoaderAlign(data.Dataset):
    def __init__(self, subject):
        super(rafdPairLoaderAlign, self).__init__()
        pairs = []
        for rafd_dir in rafd_dirs:
            for i in subject:
                if i < 10:
                    ...
                    si = str(i)
                # 对i对应的每个人，随机取一个人脸与其构成pair
                subjects = []
                for file in os.listdir(rafd_dir):
                    if file.find(si, 7, 12) != -1:
                        subjects.append(os.path.join(rafd_dir, file))
                for s in subjects:
...
def train_idnet():
    ...
    for epoch in range(51):
        dataset = rafdPairLoaderAlign(train_subject)
...
def train():
    ...
    for epoch in range(args.nepoch):
        dataset = rafdSingleLoader(train_subject)
        # print('# size of the current (sub)dataset is %d' % len(dataset))
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True,
                                                 num_workers=4)
        encoderE.train()
...
def test_classifier(iteration_count, k):
    dataset = rafdSingleTestLoader(test_subject)
    # print('# size of the current (sub)dataset is %d' % len(dataset))
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True,
                                             num_workers=4)
    encoderE.eval()
...
def test(iteration_count):
    dataset = rafdSingleLoader(test_subject)
    print('# size of the current (sub)dataset is %d' % len(dataset))
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True,
                                              num_workers=4)
...
# bestacc, bestepoch = train()
# print('bestacc is:', bestacc)
# print('bestepoch is:', bestepoch)

subject = [7, 11, 36, 46, 47, 51, 58]
expr = ['angry', 'contem', 'disgust', 'fear', 'happy', 'neutral', 'sad', 'surprise']
for e in expr:
    dirOutput = '/home/njuciairs/zmy/code/DisVAE/output/' + e
    try:
        os.makedirs(dirOutput)
    except OSError:
        pass
rafd = rafd + '/RaFD_center'
def getz_expr():
    for i in subject:
        dirOutput = 'home/njuciairs/zmy/code/DisVAE/output/'
        if i < 10:
            si = '0' + str(i)
        else:
            si = str(i)
        for file in os.listdir(rafd):
            if file.find(si, 7, 12) != -1:
                for e in expr:
                    if file.find(e) != -1:
                        label = e
                path = os.path.join(rafd, file)
                img = Image.open(path).convert('RGB')
                img = mytransform(img).cuda()
                img = torch.unsqueeze(img, dim=0)
                _, z, _ = encoderE(img)
                arr = z.detach().cpu().numpy()
                txt_path = '/home/njuciairs/zmy/code/DisVAE/output/' + label + '/' + str(i) + '_' + file.split('_')[-1][:-4]
...
def getz_id():
    for i in subject:
        dirOutput = '/home/njuciairs/zmy/code/DisVAE/output/' + str(i)
        try:
            os.makedirs(dirOutput)
        except OSError:
            pass
