...
# ident 50, facenet 10
load_encoderI = dirRoot + '/code/DisVAE/checkpoints/disvae_rafd_edit1/model/encoderI_epoch_49.pth'
# load_decoderI = dirRoot + '/code/DisVAE/checkpoints/disvae_rafd_svm/model/decoderI_epoch_49.pth'
load_encoderE = dirRoot + '/code/DisVAE/checkpoints/disvae_rafd_edit1/model/encoderE_epoch_13.pth'
load_decoderE = dirRoot + '/code/DisVAE/checkpoints/disvae_rafd_edit1/model/decoderE_epoch_13.pth'
load_mixer = dirRoot + '/code/DisVAE/checkpoints/disvae_rafd_edit1/model/mix_epoch_13.pth'
...
if args.encoder != '':
    ...
    mixer.load_state_dict(torch.load(load_mixer, map_location='cpu'))
...
def edit(face_path, exp_path):
    img_face = Image.open(face_path).convert('RGB')
    img_face = mytransform(img_face).cuda()
    img_face = torch.unsqueeze(img_face, dim=0)
    print(img_face.shape)
    _, mu_face, _ = encoderI(img_face)
    img_exp = Image.open(exp_path).convert('RGB')
    img_exp = mytransform(img_exp).cuda()
    img_exp = torch.unsqueeze(img_exp, dim=0)
    _, mu_exp, _ = encoderE(img_exp)

    muF = torch.cat((mu_face, mu_exp), 1)
    zF = mixer(muF)
    pathname = face_path.split('/')[-1][8:10] + '_' + exp_path.split('/')[-1][8:10]
    recon_save(zF, pathname)
