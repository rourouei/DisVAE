...
dirCheckpoints = dirRoot + '/code/DisVAE/checkpoints/disvae_oulu_mlp'
...
load_encoderE = dirRoot + '/code/DisVAE/checkpoints/disvae_oulu_mlp/model/encoderI_epoch_9.pth'
load_decoderE = dirRoot + '/code/DisVAE/checkpoints/disvae_oulu_mlp/model/decoderI_epoch_9.pth'
...
def train_idnet(k):
    ...
    for epoch in range(11):
        dataset = ouluPairLoaderAlign(train_split[k])
...
def train_DisVAE(k):
    ...
    for epoch in range(args.nepoch):
        dataset = ouluSingleLoader(train_split[k])
        # print('# size of the current (sub)dataset is %d' % len(dataset))
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True,
                                                 num_workers=4)
        encoderE.train()
...
def test_classifier(iteration_count, k):
    dataset = ouluSingleTestLoader(test_split[k])
    # print('# size of the current (sub)dataset is %d' % len(dataset))
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True,
                                             num_workers=4)
    encoderE.eval()
...
print('train_subject:', train_split[0])
print('test_split:', test_split[0])
print('best acc is:', train_DisVAE(0))
# # bestaccs = []
# for k in range(10):
#     print('k is:', k)
